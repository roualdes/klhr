...
class KLHR2DBAM(MCMCBase):
    ...
    def _unpack(self, eta):
        return eta[0], eta[1] ** 2, eta[2], eta[3] ** 2, eta[4]
    ...
    def logp_grad_rho(self, xi, rho):
        ...
        _, g = self.model.log_density_gradient(y)
        c = self._grad_clip
        g = np.clip(g, -c, c)
        return np.array([g.dot(rho0), g.dot(rho1)])
    ...
    def BaM(self, m, s, rho):
        ...
        for t in range(T):
            lt = 2 * 2 / (t + 1) # B * D / (t + 1)
            N = st.multivariate_normal(m, s)
            z = N.rvs(size = (B, 1))
            g = np.zeros(shape = (B, 2))
            for b in range(B):
            ...
            snew = 2 * np.linalg.solve(I + sqrtm(I + 4 * (U @ V)).T, V.T)
            m = 1 * m / (1 + lt) + lt * (snew @ gbar + zbar) / (1 + lt)
            if not np.any(~np.isfinite(s)):
                snew += I * 1e-6
                snew = 0.5 * (snew + snew.T)
                s = snew
        return m, s
    ...
    def fitBaM(self, rho):
        ...
        if h1 > 0 and np.isfinite(h1):
            s1 = h1
        m = o.x
        s = np.diag(np.array([s0, s1]))
        return self.BaM(m, s, rho)
