...
subsinhklhr = KLHRSUBSINH(bs_model, theta = pt)
...
for idx in range(4):
    ...
    qx = np.exp(sinhklhr._log_q(xx, eta))
    qx /= np.max(qx)
    zeta = klhr.fit(rho)
    px = np.exp(klhr._log_q(xx, zeta))
    px /= np.max(px)
    yeta = subsinhklhr.fit(rho)
    tx = np.exp(subsinhklhr._log_q(xx, yeta))
    tx /= np.max(tx)
    ...
    if idx == 0:
        ...
        ax2.plot(xx, px, color = "black", linestyle = "dotted", label = "klhr")
        ax2.plot(xx, tx, color = "black", linestyle = "dashdot", label = "sub")
        ax2.set_xlabel(r"$\xi$")
        ax2.legend()
    ...
    if idx == 1:
        ...
        ax3.plot(xx, tx, color = "black", linestyle = "dashdot", label = "sub")
    ...
    if idx == 2:
        ...
        ax4.plot(xx, tx, color = "black", linestyle = "dashdot", label = "sub")
    ...
    if idx == 3:
        ...
        ax5.plot(xx, tx, color = "black", linestyle = "dashdot", label = "sub")

fig.savefig("experiments/plot_funnel_general_approximations.png")
